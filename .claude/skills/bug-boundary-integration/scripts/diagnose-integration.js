  ...
  if (packageJsonExists) {
    ...
    if (hasBugReporter) {
      ...
      if (versionMatch) {
        ...
        const isLatest = version >= '1.3.0';
        checkResult(
          isLatest,
          `Using version ${version} (current is 1.3.2+)`,
          'Update to latest (1.3.x): npm install @boobalan_jkkn/bug-reporter-sdk@latest'
        );

        if (isLatest) {
          log('  ℹ v1.3.x adds network request capture, file attachments, and the LeaderboardPanel component (all optional).', 'blue');
        }
      }
    }
    ...
    allPassed &= checkResult(
      hasReactHotToast,
      'react-hot-toast installed (bundled as an SDK dependency; listing it explicitly is optional)',
      'Optional: npm install react-hot-toast'
    );
  }
