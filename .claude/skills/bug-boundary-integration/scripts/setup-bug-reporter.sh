...
install_packages() {
    ...
    print_info "Installing @boobalan_jkkn/bug-reporter-sdk (latest 1.3.x)..."
    npm install @boobalan_jkkn/bug-reporter-sdk

    # Peer dependencies: react ^18 || ^19, react-dom ^18 || ^19, lucide-react >=0.400.
    # These are typically already present in a Next.js 15+ project; install only if missing.
    print_info "Ensuring peer dependency lucide-react is present..."
    npm install lucide-react

    # react-hot-toast ships as a dependency of the SDK, so a separate install isn't
    # strictly required. We install it explicitly so you can render <Toaster /> directly.
    print_info "Installing react-hot-toast (optional; used for the <Toaster /> notifications)..."
...
main() {
    echo -e "${COLORS_CYAN}"
    echo "╔═══════════════════════════════════════════════════════╗"
    echo "║    Bug Reporter SDK Quick Setup (v1.3.x)              ║"
