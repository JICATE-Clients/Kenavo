...
- Network request capture (v1.2.0) and file attachments (v1.3.0)
- Advanced features (My Bugs Panel, Leaderboard Panel, programmatic reporting via the apiClient, custom styling)
...
### SDK Capabilities (through v1.3.x)
- Mandatory screenshot + automatic console log capture
- Network request capture (`fetch()`/XHR) with sensitive-header filtering
- File attachments (up to 5 files, 10 MB each)
- Leaderboard Panel and enhanced My Bugs Panel
- Full `apiClient` surface (createBugReport, getMyBugReports, getBugReportById, sendMessage, getLeaderboard)
...
- Support for Bug Reporter SDK v1.3.x (originally documented v1.1.0)
- Automated diagnostics and setup scripts
- Complete integration workflows
- Supabase authentication patterns
- Comprehensive troubleshooting guides

**Tracked SDK releases:** v1.1.0 (mandatory screenshots, console capture) → v1.2.0 (network request capture) → v1.3.x (file attachments, Leaderboard Panel, enhanced My Bugs Panel). All backward compatible.
