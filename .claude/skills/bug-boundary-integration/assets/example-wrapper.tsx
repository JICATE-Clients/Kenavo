...
export function BugReporterWrapper({ children }: BugReporterWrapperProps) {
  ...
  return (
    <BugReporterProvider
      ...
      // Optional network request capture (v1.2.0+). Enabled by default; sensitive
      // headers are auto-filtered and the SDK's own API calls are auto-excluded.
      // networkCapture={true}
      // networkBufferSize={10}
      // networkExcludePatterns={[/\/api\/auth\//, /supabase\.co/]}
      userContext={
        user
          ? {
              userId: user.id,
